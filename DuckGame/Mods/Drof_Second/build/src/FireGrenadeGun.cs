...
namespace MyMod.src
{
    ...
    class FireGrenadeGun : Gun
    {
        ...
        public FireGrenadeGun(float xval, float yval) : base(xval, yval)
        {
            this.ammo = 6;
            this._type = "gun";
            this._editorName = "Fire Grenade Gun";
            this._bio = "A standard grenade gun...but like with fire";
            this.graphic = new Sprite(GetPath("fireGrenadeGun"));
        ...
        public override void OnReleaseAction()
        {
            if (this._cooldown == 0f && this.ammo > 0)
            {
                ...
                {
                    FireGrenade grenade = new FireGrenade(0, 0);
                    grenade.position = Offset(new Vec2(4f, 1f));
                    grenade.HasPin = false;
                    grenade.hSpeed = this.barrelVector.x * 7f;
                    grenade.vSpeed = this.barrelVector.y * 7f;
